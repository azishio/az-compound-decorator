...
const checkAndReturnOptions = (decorators, defaultSymbolType) => {
    const checkedName = [];
    // いろいろ改変するので独立させておく
    ...
    return decArr.map((dec, decIndex) => {
        ...
        if (option.signType)
            newProp.signType = option.signType;
        checkedName.push(option.name);
        return newProp;
    });
};
exports.default = checkAndReturnOptions;
